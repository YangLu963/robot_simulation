...
from .robot_instruction import RobotInstruction  # 新增：导入结构化任务对象
...
class HierarchicalRLController:
    def __init__(self, rl_config_path: str = "configs/rl_config.yaml", device: str = 'cuda'):
        ...
        # 初始化 SAC 智能体
        obs_dim = 128  # 与 multimodal_model.fc_out 输出一致
    ...
    def execute(self, instruction: str, env: RobotEnv) -> None:
        ...
        try:
            ...
            self.logger.info(f"Task plan received: {plan.json(indent=2)}")
    ...
    def _execute_step(self, step: RobotInstruction.Step, env: RobotEnv) -> None:
        """
        执行单个子任务步骤（step 是 RobotInstruction.Step 类型）
        """
        obs = env.reset()  # 原始观测（图像 + 状态）

        for _ in range(self.config.get('max_step_per_task', 200)):
            # 1. 将原始观测和 step 指令输入多模态编码器
            obs_features = self.multimodal_encoder.encode_obs(obs, step.dict())
            ...
            # 4. 存储经验
            next_obs_features = self.multimodal_encoder.encode_obs(next_obs, step.dict())
